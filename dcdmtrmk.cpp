                           ...
                           int *NDEX )
{
   ...
   //* VALID AUTOMATED STATION CODE TYPE.  
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isRADAT( char **string, Decoded_METAR *Mptr, int *NDEX )
{
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isTornadicActiv( char **string, Decoded_METAR *Mptr, int *NDEX )
{
   int saveNdex,
       TornadicTime;
...
/*       syslog("isTornadicActivity:  current *string = %s\n",
                        *string);    */
         ...
         //  08/14/13 DDM
         //  I added this test to satisfy lint, but it looks like the code
         //  is actually checking these pointers elsewhere;
         //  I'm not actually sure that *my* test is correct here...
         // if (B_stringPtr == NULL  ||  E_stringPtr == NULL) {
         //    return FALSE;
         // }
/*
         if( B_stringPtr != NULL )
            syslog("isTornadicActivity:  B_stringPtr = %x\n", B_stringPtr);
         else
            syslog("isTornadicActivity:  B_stringPtr = NULL\n");
 
         if( E_stringPtr != NULL )
            syslog("isTornadicActivity:  E_stringPtr = %x\n", E_stringPtr);
         else
            syslog("isTornadicActivity:  E_stringPtr = NULL\n");
*/
            if( B_stringPtr != NULL && E_stringPtr == NULL ) {
...
/*          syslog("isTornadicActivity:  B_stringPtr != NULL"
               ...
               if( nisdigit((B_stringPtr+1),(E_stringPtr -
                                     (B_stringPtr+1)))) {   //lint !e613
                  TornadicTime = antoi(( B_stringPtr+1),
                                     (E_stringPtr-(B_stringPtr+1)));//lint !e613
                  if( TornadicTime > 99 ) {  
                     Mptr->BTornadicHour = TornadicTime / 100;
                     Mptr->BTornadicMinute = TornadicTime % 100;
                     (*NDEX)++;
                  ...
                  TornadicTime = antoi(( E_stringPtr+1),
                                        strlen(E_stringPtr+1)); //lint !e613
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isPartObscur( char **string, Decoded_METAR *Mptr, int ndex, int *NDEX )
{
...
#ifdef DEBUGXX
   syslog("isPartObscur:  Routine Entered...\n");
   syslog("isPartObscur:  *string = %s\n",*string);
   if( Mptr->PartialObscurationAmt[ndex][0] != '\0' ) {
      syslog("PartialObscurationAmt = %s\n",
      ...
      if( strcmp( *string, "FEW///" ) == 0 ||
          ...
          --string;
         syslog("isPartObscur:  Preceding group = %s\n",*string);
         ++string;
      }
   }
#endif
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isA0indicator( char *indicator, Decoded_METAR *Mptr, int *NDEX )
{
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isPeakWind( char **string, Decoded_METAR *Mptr, int *NDEX )
{
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isWindShift( char **string, Decoded_METAR *Mptr, int *NDEX)
{
   int temp;
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isSurfaceVsby( char **token, Decoded_METAR *Mptr, int *NDEX)
{
   char *slash;
   float S_vsby;
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isVsby2ndSite( char **token, Decoded_METAR *Mptr, int *NDEX)
{
   char *slash;
   float S_vsby,
         VSBY_2ndSite;
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isLTGfreq( char **string, Decoded_METAR *Mptr, int *NDEX )
{
   /***************************/
   /* DECLARE LOCAL VARIABLES */
   /***************************/
...
//lint -esym(838, temp)  Previously assigned value to variable 'temp' has not been used
                        ...
                        int *NDEX )
{
   ...
   while ( *temp != '\0' ) {
/*
syslog("isRecentWX:  JUST inside while-loop, *NDEX = %d\n",*NDEX);
syslog("isRecentWX:  JUST inside while-loop, temp = %s\n",temp);
*/
      i = 0;
      ...
      if( strcmp(phenom[i],"4-Zs") != 0 ) {
         ...
         if( nisdigit(numb_char,4) && strlen(numb_char) >= 4) {
            if( *C_char == 'B' ) {
               ...
               if( *NDEX < 3 ) {
                  strncpy(Mptr->ReWx[*NDEX].Recent_weather, phenom[i], (strlen(phenom[i])-1) );
                  (*NDEX)++;
               }
               if( *temp == '\0' ) {
                  free( free_temp );
...
#ifdef DEBUGQQ
   syslog("isVrbSky: *string = %s cldPtr[%d] = %s\n",
                            *string,i,cldPtr[i]);
#endif
...
void DcdMTRmk( char **token, Decoded_METAR *Mptr )
{
   ...
   if( token[ NDEX ] != NULL ) {
#ifdef DEBUGZZ
   syslog("DcdMTRmk:  RMK found, token[%d] = %s\n", NDEX,token[NDEX]);
#endif
      NDEX++;
#ifdef DEBUGZZ
   syslog("DcdMTRmk:  Bump NDEX, token[%d] = %s\n", NDEX,token[NDEX]);
#endif
   }
   else {
#ifdef DEBUGZZ
   syslog("DcdMTRmk:  No RMK found.  NULL ptr encountered\n");
#endif
      return;
   }
   /*****************************************/
...
#ifdef DEBUGZZ
   syslog("DcdMTRmk:  DECODE RMKS: token[%d] = %s\n",NDEX,token[NDEX]);
#endif
 
      isRADAT( &(token[NDEX]), Mptr, &NDEX );   //lint !e534
