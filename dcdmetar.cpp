...
/*                                                                  */
/********************************************************************/
static int ResetMETARGroup( int StartGroup, int SaveStartGroup )
{
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isSkyCond( char **skycond, Decoded_METAR *Mptr, int *NDEX )
{
...
static float prevailVSBY( char *visibility )
{
   ...
   if( Slash_ptr == NULL )
   {
      temp = (char *) malloc(sizeof(char) * ((SM_KM_ptr - visibility)+1));
      if (temp == NULL)
         return 0.0;
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isVisibility( char **visblty, Decoded_METAR *Mptr, int *NDEX )
{
   char *achar,
        *astring,
        *save_token;
   ...
   /***********************************/
               ...
               strlen(*visblty)) &&
                            strlen(*visblty) < 4 )
   {
      save_token = (char *) malloc(sizeof(char) * (strlen(*visblty)+1));
      if (save_token == NULL)
         return FALSE;
   ...
   //***************************************************
   //* CHECK FOR VISIBILITY MEASURED IN METERS, 
   //* WITH OR WITHOUT DIRECTION OF OBSERVATION      
   //***************************************************
...
/*                                                                  */
/********************************************************************/
//  08/14/13 DDM - remove static to avoid warning on unused function
// static 
MDSP_BOOL vrblVsby( char *string1, char *string2, Decoded_METAR *Mptr, int *NDEX )
{
   char buf[ 6 ];
   // int numerator,
   //     denominator;
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isMinMaxWinDir( char *string, Decoded_METAR *Mptr, int *NDEX )
{
#define buf_len 50
   char buf[ buf_len ];
   char *V_char;
...
/*                                                                  */
/********************************************************************/
static MDSP_BOOL isRVR( char *token, Decoded_METAR *Mptr, int *NDEX, int ndex )
{
                        ...
                        int *NDEX, int *next )
{
   ...
   if (temp_token_orig == NULL  ||   temp_token == NULL)
      return FALSE;
   ...
   while( temp_token != NULL && (*next) < MAXWXSYMBOLS )
   {
      ...
      else
      {
         ...
         else
         {
            ...
            temp_token = NULL;
            (*NDEX)++;
            return TRUE;   //lint !e438  temp_token
         }
...
*/
   ...
   if( charcmp(wind,"'V''R''B'dd'K''T'")) {
      ...
      (*NDEX)++;
/*
syslog("isWindData:  Passed VRBddKT test - wind = %s\n",wind);
*/
      return TRUE;
   }
   ...
   if( charcmp(wind,"'V''R''B'ddd'K''T'")) {
      ...
      (*NDEX)++;
/*
syslog("isWindData:  Passed VRBdddKT test - wind = %s\n",wind);
*/
      return TRUE;
   }
   ...
   {
/*
syslog("isWindData:  Passed 1st GUST test - wind = %s\n",wind);
*/
...
/*
syslog("isWindData:  Passed 2nd GUST test - wind = %s\n",wind);
*/
         (*NDEX)++;
         return TRUE;
      }
...
/*
syslog("isWindData:  Failed 2nd GUST test - wind = %s\n",wind);
*/
         return FALSE;
      }
   }
            ...
            ((unitsPtr-wind) >= 5 && (unitsPtr-wind) <= 6) )
   {
      ...
      (*NDEX)++;
/*
syslog("isWindData:  Passed dddff(f) test - wind = %s\n",wind);
*/
      return TRUE;
   }
   else
...
/*********************************/
   ...
   /****************************************************/
#ifdef DEBUGZZ
   syslog("DcdMETAR: Before start of tokenizing, string = %s\n", stringCpy);
#endif
...
if( strcmp(token[0],"OPKC") == 0 || strcmp(token[0],"TAPA") == 0 ) {
   syslog("DcdMETAR: token[%d]  = %s\n",NDEX,token[NDEX]);
   syslog("DcdMETAR: Token[%d]  = %s\n",NDEX,token[NDEX]);
   syslog("DcdMETAR: MetarGroup = %d\n",MetarGroup);
}
#endif
...
#ifdef DEBUGZZ
if( strcmp(token[0],"OPKC") == 0 || strcmp(token[0],"TAPA") == 0 ) {
   syslog("DcdMETAR: StartGroup = %d\n",StartGroup);
   syslog("DcdMETAR: SaveStartGroup = %d\n",SaveStartGroup);
}
#endif
...
/*          MetarGroup = SaveStartGroup;   */
            MetarGroup = (enum METAR_obGroup) ResetMETARGroup( StartGroup,
                                                               SaveStartGroup ); //lint !e64
            break;
      }  //lint !e788  enum constant 'METAR_obGroup::NotIDed' not used within defaulted switch
    }
    else
      IS_NOT_RMKS = FALSE;
...
#ifdef DEBUGZZ
if( strcmp(token[0],"OPKC") == 0 || strcmp(token[0],"TAPA") == 0 ) {
   syslog("DcdMETAR:  while loop exited, Token[%d] = %s\n",
                  NDEX,token[NDEX]);
}
#endif
      /******************************/
      ...
      /******************************/
#ifdef PRTMETAR
syslog("DCDMETAR:  Print DECODED METAR, before leaving "
...
#ifdef PRTMETAR
syslog("DCDMETAR:  Print DECODED METAR, after possible DcdMTRmk "
       "call\n\n");
prtDMETR( Mptr );
#endif
